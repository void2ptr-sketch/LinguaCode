...
- [ ] **M1** — Перенести метаданные (`knownLanguage`, `learningLanguage`, `difficulty`, `tags`) из централизованного `card-index-meta.json` в каждый соответствующий файл `data/cards/{cardId}.json`, **сохраняя 100% обратную совместимость и не ломая интерфейс**.
