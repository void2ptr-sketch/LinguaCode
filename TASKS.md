...
#### Ca2. Карточеки развите

- [ ] Все карточки должны бызироваться на:
  - вопррос;
    - изображение (иероглиф, символ, слово, предлажение и ...)
    - для языков имеющих несколько систем 
    - звук (обучающийся должен иметь возможность прослушать как это звучит на языке оригинала)
  - ответ:
- [ ] В карточках секции ответа и вопроса должны быть отделены
- [ ] В карточке должны быть возможность - открыться на весь экран (что-бы не отвлекать внимание обучающегося)
- [ ] В режиме полный экран размеры "вопросов" и "ответов" увеличиваются пропорционально размеру экрана
...
- [x] `TASK.md` - BACKLOG: задачи проекта
...
- [ ] Подсистемы проекта должны быть документированы. Формат имени файла `/docs/ARCHITECTURE.<имя подсистемы>.md`.
  - Файл должен содержать структурный дизайн каждой подситемы
  - Файлы содержат сводную информацию об особенностях подсистемы
  - Архитектурные диаграммы в файле должны быть в формате `UML`.
- [x] Файл `/docs/BUSINESS.md` должен содержать бизнес-задачи (use case) проекта
- [ ] Файл `/docs/DOMAIN.md` содержит модели проекта.
- [x] Документацию вести на русском языке.
- [ ] Формат документации UTF8.
